...
export default function ImportarInvestimentosPage() {
  ...
  const [provider, setProvider] = useState<'xp' | 'itau' | 'c6' | 'auto'>('auto');
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <select
                value={provider}
                onChange={(e) => setProvider(e.target.value as 'xp' | 'itau' | 'c6' | 'auto')}
                className="w-full px-3 py-2 text-sm rounded-lg border border-slate-200 bg-white"
              >
                <option value="auto">Auto-detectar pelo nome do arquivo</option>
                <option value="xp">XP — Posição Consolidada (.pdf)</option>
                <option value="itau">Itaú — Extrato Mensal (.pdf)</option>
                <option value="c6">C6 — Relatório Mensal (.pdf)</option>
              </select>
            </div>
        ...
        <Card>
          <CardContent>
            ...
            <ul className="space-y-1 text-xs text-slate-500">
              <li>• <strong>XP:</strong> <code className="bg-slate-100 px-1 rounded">XP_Investimentos historico_dd_mm_yyyy.pdf</code></li>
              <li>• <strong>Itaú:</strong> <code className="bg-slate-100 px-1 rounded">Itaú_Extrato Mensal_MesAAAA.pdf</code></li>
              <li>• <strong>C6:</strong> <code className="bg-slate-100 px-1 rounded">c6 investimentos DDMM.pdf</code></li>
              <li>• Importações são <strong>idempotentes</strong>: subir o mesmo arquivo novamente substitui a snapshot daquela data.</li>
              <li>• Taxas contratadas (IPCA+X%, CDI, etc.) são extraídas automaticamente.</li>
            </ul>
          </CardContent>
        </Card>
      </div>
